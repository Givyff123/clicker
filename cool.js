...
// let upg11PriceDisplay = document.querySelector("#upg11cost")
...
// let upgPrice11 = parseInt(localStorage.getItem("localStorageupgPrice11")) || 750000;
...
// upg11PriceDisplay.innerText = `cost:${upgPrice11}`
...
// upg11div.style.visibility = (localStorage.getItem("localStorageUpgrade11Visibility")) || "hidden";
...
//     }
//     if(points>1000) {
//         pointdisplay.innerText = "Cupcakes: " + Math.floor(Math.floor(points*10)/1000)/10 + "K";
//     }
//     if(points>1000000) {
//         pointdisplay.innerText = "Cupcakes: " + Math.floor(Math.floor(points*10)/1000000)/10 + "Million";
//     }
...
//     document.querySelector("#upg10div").style.visibility = "hidden"
...
//     localStorage.setItem("localStorageUpgrade10Visibility", "hidden")
...
//         upg11PriceDisplay.innerText = `cost:750000`
//         localStorage.setItem("localStorageupgPrice1", `${upgPrice1}`)
//         localStorage.setItem("localStorageupgPrice2", `${upgPrice2}`)
//         localStorage.setItem("localStorageupgPrice3", `${upgPrice3}`)
//         localStorage.setItem("localStorageupgPrice4", `${upgPrice4}`)
...
//         localStorage.setItem("localStorageupgPrice11", `${upgPrice11}`)
//         update()
...
//         localStorage.setItem("localStorageupgPrice10", `${upgPrice10}`)
//         upg10PriceDisplay.innerText = `cost:${upgPrice10}`
//         document.querySelector("#upg11div").style.visibility = "visible"
//         update()
//         cash.play()
//     }
// });

// document.querySelector("#upgrade11").addEventListener("click", () => {
//     if (points > upgPrice11-1) {
//         pointsPerSec += 10000
//         points -= upgPrice11;
//         upgPrice11+=Math.round(upgPrice11/10)
//         localStorage.setItem("localStorageupgPrice11", `${upgPrice11}`)
//         upg11PriceDisplay.innerText = `cost:${upgPrice11}`
//         update()
//         cash.play()
...
//         multiplier+=1
//         pp-=1
//         pointsPerClick = pointsPerClick/(multiplier-1)
//         pointsPerClick = pointsPerClick*multiplier
// }
// })
...
//     localStorage.setItem("localStorageUpgrade11Visibility", `${document.querySelector("#upg11div").style.visibility}`)
...
const gameState = {
    ...
    multiplier: 1,
    upgrades: {
        oven: { cost: 100, effect: () => gameState.cpc += 0.5 },
        nuts: { cost: 50, effect: () => gameState.cps += 0.5 },
        chocolateChips: { cost: 550, effect: () => gameState.cps += 6 },
        epicOven: { cost: 600, effect: () => gameState.cpc += 3.5 },
        butter: { cost: 5000, effect: () => gameState.cps += 62.5 },
        legendaryOven: { cost: 20000, effect: () => gameState.cpc += 62.5 },
        meltedChocolate: { cost: 50000, effect: () => gameState.cps += 300 },
        bigCupcakeBang: { cost: 150000, effect: () => {gameState.prestigePoints+=1
                                                       gameState.visibility.heaven = "visible"
                                                       setTimeout(()=>{
                                                        gameState.visibility.heaven = "hidden"
                                                       }, 3000)
        }},
        research: {cost: 150000, effect: () => gameState.visibility.mythicalOven = "visible"},
        mythicalOven: {cost: 300000, effect: () => {gameState.cpc += 5000 
                                    gameState.visibility.caramel = "visible" }},
        caramel: {cost: 750000, effect: () => gameState.cps += 10000 }
    },
    visibility: {
        butter: "hidden",
        chocolate: "hidden",
...
document.getElementById('button').addEventListener('click', () => {
    gameState.cupcakes += gameState.cpc * gameState.multiplier;
    updateUI();
    saveGame();
});
...
document.querySelectorAll('.upgdivs button').forEach(button => {
    i++
    console.log(i)
    button.addEventListener('click', () => {
        const upgrade = gameState.upgrades[i];
        if (gameState.cupcakes > upgrade.cost-1) {
            gameState.cupcakes -= upgrade.cost;
            upgrade.effect();
            updateUI();
            saveGame();
        }
...
function updateUI() {  
    document.querySelector('#pointdisplay').innerText = `Cupcakes: ${Math.floor(gameState.cupcakes*10)/10}`;
    document.querySelector('#pointspersecdisplay').innerText = `Cps: ${gameState.cps}`;
    document.querySelector('#pointsperclickdisplay').innerText = `Cpc: ${gameState.cpc}`;
    document.querySelector('#ppdisplay').innerText = `Prestige Points: ${gameState.prestigePoints}`;
    document.querySelector('#upg11div').style.visibility = gameState.visibility.caramel;
}
...
setInterval(()=>{
    gameState.cupcakes += gameState.cps/10;
    updateUI();
}, 100)
